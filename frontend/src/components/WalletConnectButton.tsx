...
export default function WalletConnectButton() {
  ...
  return (
    <div className="flex items-center gap-3 relative">
      ...
      {error && (
        <div role="alert" className="absolute top-14 right-0 w-72 p-4 bg-red-500/10 border border-red-500/30 rounded-xl glass shadow-xl z-50 animate-in slide-in-from-top-2">
          <div className="flex items-start gap-3">
            <svg className="w-5 h-5 text-red-400 shrink-0 mt-0.5" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button onClick={() => setError(null)} aria-label="Dismiss wallet connection error" className="text-red-400/50 hover:text-red-400 transition-colors">
      ...
      {account ? (
        ...
        >
          <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-indigo-500/20 to-purple-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" aria-hidden="true"></div>
          <span className="relative text-sm font-semibold tracking-wide text-white flex items-center gap-2">
            {isConnecting ? (
              <>
                <svg className="animate-spin h-4 w-4 text-indigo-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
